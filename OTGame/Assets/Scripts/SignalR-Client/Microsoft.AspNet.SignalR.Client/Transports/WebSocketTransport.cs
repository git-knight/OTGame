...
namespace Microsoft.AspNet.SignalR.Client.Transports
{
    public class WebSocketTransport : ClientTransportBase
    {
        ...
        {
            return PerformConnect(_connection.Url.Substring(0, _connection.Url.Length - 1) + "?id=" + _connection.ConnectionId);
        ...
        private async Task PerformConnect(string url)
        {
            ...
            var buffer = Encoding.UTF8.GetBytes("{\"protocol\":\"json\",\"version\":1}\x1e");
            await _webSocket.SendAsync(new ArraySegment<byte>(buffer), WebSocketMessageType.Text, true, CancellationToken.None);
            await _webSocketHandler.ProcessWebSocketRequestAsync(_webSocket, token);
            //await _webSocketHandler.SendAsync();
        }
        ...
        internal virtual void OnOpen()
        {
            ...
            if (_connection.State == ConnectionState.Reconnecting && _connection.ChangeState(ConnectionState.Reconnecting, ConnectionState.Connected))
                _connection.OnReconnected();

            if (_connection.State == ConnectionState.Connecting && _connection.ChangeState(ConnectionState.Connecting, ConnectionState.Connected))
                _connection.OnReconnected();
        }
